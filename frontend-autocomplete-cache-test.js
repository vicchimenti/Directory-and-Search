...
export default function () {
  ...
  // Construct a URL with the query embedded in parameters
  const url = `${BASE_URL}/search-test?query=${encodeURIComponent(query)}&sessionId=${sessionId}`;

  const res = http.get(url);

  check(res, {
    'status is 200': (r) => r.status === 200,
    'contains result container': (r) => r.body.includes('funnelback-search-container'),
  });

  sleep(Math.random() * 6 + 5); // 5–11 sec pacing to stay under rate limit
}
